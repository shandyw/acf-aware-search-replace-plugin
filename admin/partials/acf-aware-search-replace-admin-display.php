...
// --- Stable total results (prefer global total computed in handle_run) ---
$cached_total = get_transient('acfsr_last_total_rows');

// Fallback chain:
// 1) use the cached global total if present
// 2) else use summary['total_rows'] from the last scan
// 3) else, as a last resort, count the cached rows array (may be capped)
if (is_numeric($cached_total)) {
    $total_rows = (int) $cached_total;
} elseif (isset($summary['total_rows'])) {
    $total_rows = (int) $summary['total_rows'];
} elseif (is_array($all_rows)) {
        ...
        <table class="widefat striped">
            <thead>
                <tr>
                    ...
                    <th><?php esc_html_e('URL', 'acf-aware-search-replace'); ?></th>
                        ...
                        <td>
                            <?php if (!empty($r['url'])): ?>
                                <a href="<?php echo esc_url($r['url']); ?>" target="_blank" rel="noopener">
                                    <?php esc_html_e('View', 'acf-aware-search-replace'); ?>
                                </a>
                                <br><code style="word-break:break-all;"><?php echo esc_html($r['url']); ?></code>
                            <?php endif; ?>
                        </td>
